...
void rounds(unsigned int seed, int transient_period, int customers_number, int round_number, float lambda, bool preemption, bool allow_logging){
	...
	// Since we must ignore the first transient_period customers, only customers considered will be the ones with id >= 0
	...
	for (int round = 0; round < round_number+1; round++) {
		...
		// Loops until customers_number customers are sampled or, if this is round 0, until the transient_period customers are sampled
		while ((round > 0 && Customer::total_customers < customers_number * round) || (round == 0 && Customer::total_customers < 0)) {
			...
			current_event.treat_event(round, data_traffic, voice_traffic, &customer_being_served, preemption);
			...
			if (current_event.etype == ARRIVAL && current_event.customer.type == DATA) {
				...
				if (voice_channels[current_event.channel_id] > 0) {
					...
					list_insert(event_list, create_voice(-99999, simulation_time, 16, current_event.channel_id)); // next voice package of this channel
				} else {
				...
				if (voice_channels[current_event.channel_id] > 0) {
					voice_channels[current_event.channel_id]--;
					list_insert(event_list, create_voice(-99999, simulation_time, 0, current_event.channel_id)); // next voice package of this channel
				}
...
void execution(unsigned int seed, int transient_period, int customers_number, int round_number, float utilization1, bool preemption, bool allow_logging){
    ...
    // Starts rounds
    rounds(seed, transient_period, customers_number, round_number, lambda, preemption, allow_logging);
}
